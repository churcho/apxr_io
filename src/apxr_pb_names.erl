...
-spec encode_msg(#'Names'{} | #'Project'{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 ->
    encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#'Names'{} | #'Project'{}, atom() | list()) -> binary().
...
encode_msg_Names(#'Names'{projects = F1,
			  repository = F2},
		 Bin, TrUserData) ->
    B1 = begin
...
encode_msg_Project(#'Project'{name = F1}, Bin,
		   TrUserData) ->
    begin
      TrF1 = id(F1, TrUserData),
...
dfp_read_field_def_Names(<<10, Rest/binary>>, Z1, Z2,
    ...
    d_field_Names_projects(Rest, Z1, Z2, F@_1, F@_2,
			   TrUserData);
...
dfp_read_field_def_Names(<<>>, 0, 0, R1, F@_2,
			 TrUserData) ->
    #'Names'{projects = lists_reverse(R1, TrUserData),
	     repository = F@_2};
...
dg_read_field_def_Names(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
      10 ->
	  d_field_Names_projects(Rest, 0, 0, F@_1, F@_2,
				 TrUserData);
      18 ->
...
dfp_read_field_def_Project(<<10, Rest/binary>>, Z1, Z2,
			   F@_1, TrUserData) ->
    d_field_Project_name(Rest, Z1, Z2, F@_1, TrUserData);
dfp_read_field_def_Project(<<>>, 0, 0, F@_1, _) ->
    #'Project'{name = F@_1};
...
dg_read_field_def_Project(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
      10 ->
	  d_field_Project_name(Rest, 0, 0, F@_1, TrUserData);
      _ ->
	  case Key band 7 of
...
dg_read_field_def_Project(<<>>, 0, 0, F@_1, _) ->
    #'Project'{name = F@_1}.
...
d_field_Project_name(<<0:1, X:7, Rest/binary>>, N, Acc,
    ...
    dfp_read_field_def_Project(RestF, 0, 0, NewFValue,
			       TrUserData).
...
merge_msg_Names(#'Names'{projects = PFprojects},
		#'Names'{projects = NFprojects,
...
merge_msg_Project(#'Project'{},
		  #'Project'{name = NFname}, _) ->
    #'Project'{name = NFname}.
...
v_msg_Names(#'Names'{projects = F1, repository = F2},
	    Path, TrUserData) ->
    if is_list(F1) ->
...
v_msg_Project(#'Project'{name = F1}, Path,
	      TrUserData) ->
