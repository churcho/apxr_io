...
-spec encode_msg(versions() | project(), atom()) -> binary().
...
encode_msg_project(#{name := F1} = M, Bin,
		   TrUserData) ->
    B1 = begin
	   TrF1 = id(F1, TrUserData),
...
dfp_read_field_def_versions(<<>>, 0, 0, R1, F@_2,
    ...
    S1 = #{repository => F@_2},
    if R1 == '$undef' -> S1;
       true -> S1#{projects => lists_reverse(R1, TrUserData)}
    end;
...
merge_msg_versions(#{} = PMsg,
    ...
    case {PMsg, NMsg} of
      ...
      {_, #{projects := NFprojects}} ->
	  S1#{projects => NFprojects};
      {#{projects := PFprojects}, _} ->
	  S1#{projects => PFprojects};
      {_, _} -> S1
    end.

-compile({nowarn_unused_function,merge_msg_project/3}).
merge_msg_project(#{} = PMsg, #{name := NFname} = NMsg,
		  TrUserData) ->
    S1 = #{name => NFname},
...
v_msg_project(#{name := F1} = M, Path, TrUserData) ->
