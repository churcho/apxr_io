...
encode_msg_Signed(#'Signed'{payload = F1,
			    signature = F2},
		  Bin, TrUserData) ->
    B1 = begin
...
dfp_read_field_def_Signed(<<>>, 0, 0, F@_1, F@_2, _) ->
    #'Signed'{payload = F@_1, signature = F@_2};
...
merge_msg_Signed(#'Signed'{signature = PFsignature},
		 #'Signed'{payload = NFpayload, signature = NFsignature},
		 _) ->
    #'Signed'{payload = NFpayload,
	      signature =
...
v_msg_Signed(#'Signed'{payload = F1, signature = F2},
	     Path, TrUserData) ->
...
get_msg_defs() ->
    [{{msg, 'Signed'},
      [#field{name = payload, fnum = 1, rnum = 2,
