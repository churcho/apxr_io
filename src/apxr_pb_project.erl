...
-type 'RetirementStatus'() :: #'RetirementStatus'{}.

-export_type(['Project'/0, 'Release'/0, 'RetirementStatus'/0]).

-spec encode_msg(#'Project'{} | #'Release'{} | #'RetirementStatus'{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 ->
    encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#'Project'{} | #'Release'{} | #'RetirementStatus'{}, atom() | list()) -> binary().
...
encode_msg_Project(#'Project'{releases = F1, name = F2,
			      repository = F3},
		   Bin, TrUserData) ->
    B1 = begin
...
encode_msg_Release(#'Release'{version = F1,
			      checksum = F2, retired = F3},
		   Bin, TrUserData) ->
    B1 = begin
	   TrF1 = id(F1, TrUserData),
...
encode_msg_RetirementStatus(#'RetirementStatus'{reason =
						    F1,
						message = F2},
			    Bin, TrUserData) ->
    B1 = begin
...
dfp_read_field_def_Project(<<>>, 0, 0, R1, F@_2, F@_3,
			   TrUserData) ->
    #'Project'{releases = lists_reverse(R1, TrUserData),
...
dfp_read_field_def_Release(<<>>, 0, 0, F@_1, F@_2, F@_3,
			   _) ->
    #'Release'{version = F@_1, checksum = F@_2,
	       retired = F@_3};
...
dfp_read_field_def_RetirementStatus(<<>>, 0, 0, F@_1,
				    F@_2, _) ->
    #'RetirementStatus'{reason = F@_1, message = F@_2};
...
merge_msg_Project(#'Project'{releases = PFreleases},
		  #'Project'{releases = NFreleases, name = NFname,
			     repository = NFrepository},
		  TrUserData) ->
    #'Project'{releases =
		   if PFreleases /= undefined, NFreleases /= undefined ->
			  'erlang_++'(PFreleases, NFreleases, TrUserData);
		      PFreleases == undefined -> NFreleases;
		      NFreleases == undefined -> PFreleases
		   end,
	       name = NFname, repository = NFrepository}.

-compile({nowarn_unused_function,merge_msg_Release/3}).
merge_msg_Release(#'Release'{retired = PFretired},
		  #'Release'{version = NFversion, checksum = NFchecksum,
			     retired = NFretired},
		  TrUserData) ->
    #'Release'{version = NFversion, checksum = NFchecksum,
	       retired =
...
merge_msg_RetirementStatus(#'RetirementStatus'{message =
						   PFmessage},
			   #'RetirementStatus'{reason = NFreason,
					       message = NFmessage},
			   _) ->
    #'RetirementStatus'{reason = NFreason,
			message =
...
v_msg_Project(#'Project'{releases = F1, name = F2,
			 repository = F3},
	      Path, TrUserData) ->
    if is_list(F1) ->
...
v_msg_Release(#'Release'{version = F1, checksum = F2,
			 retired = F3},
	      Path, TrUserData) ->
    ...
    if F3 == undefined -> ok;
       true ->
	   v_msg_RetirementStatus(F3, [retired | Path], TrUserData)
    end,
    ok;
...
v_msg_RetirementStatus(#'RetirementStatus'{reason = F1,
					   message = F2},
		       Path, TrUserData) ->
