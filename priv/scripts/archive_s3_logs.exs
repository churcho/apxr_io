...
buckets = [{"logs.apxr.io", "us-east-1"}]
...
keys =
  ...
  |> Stream.map(fn {bucket, region, day} ->
    ...
    prefix = "#{dir}/#{date}-#{day}"
    {bucket, region, ApxrIo.Store.S3.list(region, bucket, prefix)}
  end)
  ...
  File.open!(filename, [:write, :delayed_write, :compressed], fn file ->
    ...
    |> Task.async_stream(
      fn {bucket, region, key} ->
        data =
          ApxrIo.Store.S3.get(region, bucket, key, [])
          |> uncompress.(key)
...
if action == "upload" || action == "fetch-and-upload" do
  ...
  |> ExAws.S3.upload("backup.apxr_io.com", key, timeout: 600_000)
  |> ExAws.request!(region: "us-east-1")
end

if action == "delete" do
  Enum.each(keys, fn {bucket, region, keys} ->
    ApxrIo.Store.S3.delete_many(region, bucket, keys)
  end)
end
